...
from network_securities.entity.config_entity import DataValidationConfig, TraingPipelineConfig
...
from network_securities.utils.main_utils.utils import read_yaml_file, write_yaml_file
...
class DataValidation:
    ...
    def detect_dataset_drift(self, base_df: pd.DataFrame, current_df: pd.DataFrame, threshold=0.05) -> bool:
        try:
            ...
            for column in base_df.columns:
                ...
                statistic = ks_2samp(base_data, current_data)
                if threshold <= statistic.pvalue:
                    is_found = False
                else:
                    is_found = True
                    status = False
    ...
    def initiate_data_validation(self) -> DataValidationConfig:
        try:
            ...
            if not status:
                raise Exception("Train dataframe does not have all columns")

            status = self.validate_number_of_columns(dataframe=test_df)
            if not status:
                raise Exception("Test dataframe does not have all columns")

            ## let's validate the data drift
